...
public class PreprocessModel implements EntitiveContract.Model {

    private Activity activity;

    public PreprocessModel(Activity activity) {
    ...
    public void getEntitve(String pagNum, String limits, final PreprocessCallBack callBack){
        Api.preprocessinfo(pagNum, limits, new DialogCallBack<Preprocess>(activity) {
            @Override
    ...
    public  void checked(String examine_processId,final PreprocessCallBack callBack){
        Api.preprocesschecked(examine_processId, new DialogCallBack<Preprocess>(activity) {
            @Override
            public void onSuccess(Response<Preprocess> response) {
                if(response.body()!=null){
                    if(response.body().getStatus().equals("1")) {
    ...
    //逐级审核
    public  void nextchecked(String examine_id,String examine_processId,String currentTaskId,String auditState,String rejectReason,String allowDownload,final PreprocessCallBack callBack){
        Api.preprocessnextchecked(examine_id, examine_processId, currentTaskId, auditState, rejectReason, allowDownload, new DialogCallBack<Preprocess>(activity) {
            @Override
            public void onSuccess(Response<Preprocess> response) {
                if(response.body()!=null){
                    if(response.body().getStatus().equals("1")) {
